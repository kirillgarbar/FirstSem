...
type QuadTreeMatrix<'t when 't : equality> =
    ...
    static member mul (m1:QuadTreeMatrix<'t>) (m2:QuadTreeMatrix<'t>) (astruct:AlgebraicStruct<'t>) =
        ...
        let rec go (m1:QuadTree<'t>) (m2:QuadTree<'t>) currSize =
            ...
            | _, None -> None
            | Node(nw1, ne1, se1, sw1), Node(nw2, ne2, se2, sw2) ->
                let nw = QuadTreeMatrix.sum (QuadTreeMatrix(currSize, currSize, go nw1 nw2 (currSize/2))) (QuadTreeMatrix(currSize, currSize, go ne1 sw2 (currSize/2))) astruct
                let ne = QuadTreeMatrix.sum (QuadTreeMatrix(currSize, currSize, go nw1 ne2 (currSize/2))) (QuadTreeMatrix(currSize, currSize, go ne1 se2 (currSize/2))) astruct
                let se = QuadTreeMatrix.sum (QuadTreeMatrix(currSize, currSize, go sw1 ne2 (currSize/2))) (QuadTreeMatrix(currSize, currSize, go se1 se2 (currSize/2))) astruct
                let sw = QuadTreeMatrix.sum (QuadTreeMatrix(currSize, currSize, go sw1 nw2 (currSize/2))) (QuadTreeMatrix(currSize, currSize, go se1 sw2 (currSize/2))) astruct
    ...
    member this.scalarMul s (astruct:AlgebraicStruct<'t>) =
        ...
        QuadTreeMatrix(this.Cols, this.Rows, if s = neutral then None else go this.Tree)
        
    static member tensorMul (m1:QuadTreeMatrix<'t>) (m2:QuadTreeMatrix<'t>) (astruct:AlgebraicStruct<'t>) =
        let rec go m1 =
            match m1 with
            | Leaf x -> (m2.scalarMul x astruct).Tree
            | None -> None
            | Node(nw, ne, se, sw) ->
                let nw = go nw
                let ne = go ne

...
let rec fold folder acc l  =
    match l with
    | Single x -> folder x acc
    | Cons(x, tail) -> fold folder (folder x acc) tail

let rec concat l1 l2 =
    match l1 with
        | Single last -> Cons(last, l2)
...
let bubbleSort l =
    let rec go l =
        match l with
        | Single el -> Single el
        | Cons(head, Single el) -> if head <= el then l else Cons(el, Single head)
...
let listToMyList l =
    ...
    then
        ...
        go (List.rev l).Tail (Single l.[l.Length - 1])

let myListToList l =
    fold (fun x list -> list @ [x]) [] l

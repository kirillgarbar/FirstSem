...
let genRandomList count =
    ...
    let c = if count = 0 then rnd.Next(100) else abs count
...
let tests1 =
    testList "Tests for Mytree" [
        testCase "max test" <| fun a ->
            let tree = Node(15, Cons(Node(5, Single(Leaf 56)), Single(Node(7, Single(Leaf 30)))))
            Expect.equal 56 (max tree) "max is incorrect"
        testCase "average test" <| fun a ->
            let tree = Node(15, Cons(Node(5, Single(Leaf 56)), Single(Node(7, Single(Leaf 30)))))
